	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="connectTable">
		<tbody><tr>
			<td align="right" width="110" valign="middle">
					...
					<tr>
						<td colspan="2">
							<div class="alignC" style="font-size: 12px; margin-bottom: 3px" bis_skin_checked="1"><a id="user-connect-vanity-url" href="<?php if(empty($user['vanity'])) { echo "/user/".htmlspecialchars($username); } else{ echo "/".$user['vanity']; } ?>">http://bittube.great-site.net/<?php if(empty($user['vanity'])) { echo "/user/".htmlspecialchars($username); } else{ echo "/".$user['vanity']; } ?></a></div>
						</td>
					</tr>
					<tr>
</tbody></table>
