...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if (strpos($param_username, 'hwd') !== false) { die(''); }
...
<body>
    <div class="wrapper">
        <h2>Sign Up to BitTube</h2>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>?debug" method="post">
